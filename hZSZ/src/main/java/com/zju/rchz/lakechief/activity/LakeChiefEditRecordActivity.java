...
import com.zju.rchz.chief.activity.ChiefTrackViewActivity;
...
public class LakeChiefEditRecordActivity extends BaseActivity {
    ...
    //LakeRecord:巡湖相关类
    private LakeRecord riverRecord = null; //因为修改太多，此处用riverRecord作为一个  LakeRecord实例
    ...
    private boolean isNew; //是否是新建巡湖单

    //倒计时
    private LakeChiefEditRecordActivity.TimeCount time;
    private Button submit;//提交巡湖记录
...
//        findViewById(R.id.btn_selriver).setTag("tt:{.locLakeName}");
        ...
        btn_track.setOnClickListener(new View.OnClickListener() {
            ...
            public void onClick(View v) {
                //1224
                Intent intent = new Intent(LakeChiefEditRecordActivity.this, ChiefInspectActivity.class);
        ...
        btn_trackView.setOnClickListener(new View.OnClickListener() {
            ...
            public void onClick(View v) {
                //1224
                Intent intent = new Intent(LakeChiefEditRecordActivity.this, ChiefTrackViewActivity.class);
        ...
        riverRecord = StrUtils.Str2Obj(getIntent().getStringExtra(Tags.TAG_RECORD), LakeRecord.class);
        ...
        if (riverRecord == null) {//从新建中进入
            ...
            handler.postDelayed(new LakeChiefEditRecordActivity.MyRunable(), 1000); //每10s记录一次当前轨迹
            //只有在新建时才记录轨迹
            riverRecord = new LakeRecord();
            riverRecord.recordDate = DateTime.getNow(); //巡湖时间
            riverRecord.locLakeName = "选择湖泊";
...
//			System.out.println("startTime: " + startTime);
            ...
            if (getUser().getBaiduLatPoints() != null && !getUser().getBaiduLatPoints().equals("")) {
                ...
                time = new LakeChiefEditRecordActivity.TimeCount(300000, 1000);//构造CountDownTimer对象，倒计时
                time.start();//开始计时
...
//            findViewById(R.id.btn_selriver).setVisibility(View.GONE);
            ...
            showOperating();
            getRequestContext().add("Get_LakeRecord_Content", new Callback<LakeRecordRes>() {
                @Override
...
////                        o.data.isRead = riverRecord.isRead;
                        ...
                        riverRecord.locLake = null;
                        for (Lake r : getUser().lakeSum) {
                            if (riverRecord.lakeId == r.lakeId) {
                                riverRecord.locLake = r;
                                riverRecord.locLakeName = r.lakeName;
                            }
                        }
...
//                        else
            ...
            }, LakeRecordRes.class, ParamUtils.freeParam(null, "recordId", riverRecord.recordId));
        }
    ...
    private CompoundButton.OnCheckedChangeListener cclTogTag = new CompoundButton.OnCheckedChangeListener() {
        @Override
    ...
    private View.OnClickListener exitTrackRiver = new View.OnClickListener() {
        ...
        public void onClick(View view) {
            AlertDialog.Builder ab = new AlertDialog.Builder(LakeChiefEditRecordActivity.this);
            ab.setTitle("是否退出巡湖");
            ab.setMessage("退出巡湖界面后，所记录轨迹将消失");
    ...
    private void refreshSelectLakeBtn() {
        if (riverRecord.locLake != null){
            ((Button) findViewById(R.id.btn_selriver)).setText(riverRecord.locLake.lakeName);
        }else if (getUser().lakeSum.length == 1) {
            //如果湖长或人大只有一个湖，则直接显示其所负责的湖泊
            ((Button) findViewById(R.id.btn_selriver)).setText(getUser().lakeSum[0].lakeName);
            riverRecord.locLake = getUser().lakeSum[0];
            riverRecord.lakeId = riverRecord.locLake.lakeId;
            riverRecord.locLakeName = riverRecord.locLake.lakeName;
        } else{
    ...
    private void selectLake() {
        ...
        Dialog alertDialog = new AlertDialog.Builder(this).setTitle("请选择湖泊").setItems(names, new DialogInterface.OnClickListener() {
            ...
            public void onClick(DialogInterface dialog, int which) {
                riverRecord.locLake = getUser().lakeSum[which];
                riverRecord.lakeId = riverRecord.locLake.lakeId;
                riverRecord.locLakeName = riverRecord.locLake.lakeName;
            ...
            case R.id.btn_submit: {
                ...
                if (riverRecord.lakeId == 0) {
                    ...
                    try {
                        ...
                        submitParam.put("lakeId", riverRecord.lakeId);
...
//									Log.i("NET", bmp.toString() + " bts.len " + bts.length);
    ...
    public LakeChiefEditRecordActivity.MyLocationListener myListener = new LakeChiefEditRecordActivity.MyLocationListener();
...
//                }
                if (getDistance(point,points.get(points.size()-1))<150 &&
