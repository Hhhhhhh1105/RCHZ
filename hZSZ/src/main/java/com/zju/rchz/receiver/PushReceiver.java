...
public class PushReceiver extends BroadcastReceiver {
	...
	private static String payloadData = null;

	public static Object getPayload() {
		return payloadData != null && payloadData.length() > 0 ? payloadData : null;
	}
	...
	public void onReceive(Context context, Intent intent) {
		...
		case PushConsts.GET_MSG_DATA:
			...
			if (payload != null) {
				...
				if (BaseActivity.getCurActivity() instanceof BaseActivity) {
					BaseActivity.getCurActivity().checkChiefNotify();
				}
			} else {
				payloadData = "";
			}
