...
/**
 * Created by DONGH on 2017/9/1.
 */

public class DuChaListAcitivity extends BaseActivity implements ListViewWarp.WarpHandler {
        ...
        adapter = new SimpleListAdapter(this, comps, new SimpleViewInitor() {
            ...
            public View initView(Context context, int position, View convertView, ViewGroup parent, Object data) {
                if (convertView == null) {
                    ...
                    convertView = LinearLayout.inflate(DuChaListAcitivity.this, R.layout.item_complaint, null);
                }
                CompSugs comp = (CompSugs) data;
                ViewWarp warp = new ViewWarp(convertView, DuChaListAcitivity.this);
                warp.setText(R.id.btn_vieworder,"查看详情");
                warp.setText(R.id.tv_title, comp.getCompTheme());
                warp.setText(R.id.tv_content, comp.getComplaintsContent());
                warp.setText(R.id.tv_sernum, comp.getComplaintsNum());
...
//                }
                ...
                warp.getViewById(R.id.btn_evaluatenow).setVisibility(View.GONE);
...
//                    Intent intent = new Intent(ProblemReportListActivity.this, EvalCompActivity.class);
//                    intent.putExtra(Tags.TAG_COMP, StrUtils.Obj2Str(comp));
//                    startActivityForResult(intent, Tags.CODE_COMP);
//                } else
                if (v.getId() == R.id.btn_vieworder) {
                    Intent intent = new Intent(DuChaListAcitivity.this, DuChaDetailActivity.class);
                    intent.putExtra(Tags.TAG_COMP, StrUtils.Obj2Str(comp));
...
//        if(comps.size()>1){
//            Toast.makeText(getCurActivity(),"dayu1",Toast.LENGTH_LONG).show();
//        }else {
//            Toast.makeText(getCurActivity(),"000",Toast.LENGTH_LONG).show();
//        }
        loadComps(true);
        return true;
    }
    ...
    private void loadComps(final boolean refresh) {
        ...
        getRequestContext().add("ducha_list_get", new Callback<CompDataRes>() {
            @Override
            public void callback(CompDataRes o) {
                ...
                if (o != null && o.isSuccess()) {
                    CompSugs[] list = o.data.complaints;
                    if (list != null) {
                        if (refresh) {
                            comps.clear();
                        }
