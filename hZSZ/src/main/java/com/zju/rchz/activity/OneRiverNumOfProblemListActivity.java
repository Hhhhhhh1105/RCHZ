...
import com.zju.rchz.chief.activity.DuabnTopersonCitychiefDetailActivity;
 ...
 * 具体某条河 为批示的问题列表
 * false:
 * 具体某个人的所有河道的问题列表(镇街总河长以及市级河长)
 *
 */

public class OneRiverNumOfProblemListActivity extends BaseActivity implements ViewPager.OnPageChangeListener, RadioGroup.OnCheckedChangeListener{
    private River river = null;
    private boolean isOneRiver = false;
        ...
        private View.OnClickListener btnClk = new View.OnClickListener() {
            ...
            public void onClick(View v) {
                if (v.getTag() instanceof DubanTopersonObject) {
                    ...
                    Intent intent = new Intent(OneRiverNumOfProblemListActivity.this,
                            LeaderIntructionDetailActivity.class);

                    intent.putExtra(Tags.TAG_COMP, StrUtils.Obj2Str(comp));
                    intent.putExtra(Tags.TAG_ISCOMP, isComp);
                    intent.putExtra(Tags.TAG_HANDLED, true);    //
...
//                warp.setText(R.id.tv_status, comp.getStatuss());  //处理状态
        ...
        public View getView() {
            if (view == null) {
                view = LinearLayout.inflate(OneRiverNumOfProblemListActivity.this,
                        R.layout.confs_chief_complist, null);
                adapter = new SimpleListAdapter(OneRiverNumOfProblemListActivity.this,
                        items, initor);
                listViewWarp = new ListViewWarp(OneRiverNumOfProblemListActivity.this,
                        adapter, this);
    ...
    private int isCityChief = 0;
    private int isLeaderDuban=0;
...
//        isLeaderDuban=getIntent(),getExtra()

        setTitle(R.string.instruction);
        Button button_handling=(Button)findViewById(R.id.rb_chief_comp_unhandle);
        button_handling.setText("处理中");
        button_handling.setVisibility(View.GONE);
        Button button_handled = (Button)findViewById(R.id.rb_chief_comp_handled);
        button_handled.setText("已处理");
        button_handled.setVisibility(View.GONE);
        initHead(R.drawable.ic_head_back, 0);

        river = StrUtils.Str2Obj(getIntent().getStringExtra(Tags.TAG_RIVER), River.class);//传输的河道信息
        isOneRiver = getIntent().getBooleanExtra("isOneRiver",false);
        ...
        pagerItems.add(new OneRiverNumOfProblemListActivity.CompListPager(0));
        pagerItems.add(new OneRiverNumOfProblemListActivity.CompListPager(1));
...
//        }
    ...
    protected void onActivityResult(int requestCode, int resultCode, Intent data) {
        ...
        if (resultCode == RESULT_OK) {
            ((OneRiverNumOfProblemListActivity.CompListPager) pagerItems
